// ─── File upload interactive widget ──────────────────────────────────────────

import { UploadCloud, FileImage, X } from "lucide-react";
import { useState } from "react";

export function FileUploadPreview({ field }: { field: FormField }) {
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        const names = field.multiple ? files.map((f) => f.name) : [files[0].name];
        setDroppedFiles((prev) => (field.multiple ? [...prev, ...names] : names));
    };
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const files = Array.from(e.target.files ?? []);
        const names = field.multiple ? files.map((f) => f.name) : files.slice(0, 1).map((f) => f.name);
        setDroppedFiles((prev) => (field.multiple ? [...prev, ...names] : names));
    };
                        ...
                        <div className="flex flex-wrap justify-center gap-1 mt-1">
                            {field.acceptedTypes.map((t) => (
                                <span
                                    ...
                                    style={{ fontSize: "0.62rem", fontWeight: 600, background: "var(--secondary)", color: "var(--muted-foreground)", letterSpacing: "0.04em" }}
                                >
                                    {t}
                                </span>
                            ))}
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex flex-col min-w-0 flex-1">
                                    <span style={{ fontSize: "0.75rem", fontWeight: 500, color: "var(--foreground)" }} className="truncate">
                                        {name}
                                    </span>
                                    <span style={{ fontSize: "0.65rem", color: "var(--muted-foreground)" }}>{ext} file</span>
                                </div>
                                <button
                                    onClick={(e) => {
                                        e.preventDefault();
