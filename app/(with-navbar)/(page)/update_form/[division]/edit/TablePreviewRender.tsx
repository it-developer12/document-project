// ─── Table field interactive preview ─────────────────────────────────────────
...
function TableCellInput({ col, value, onChange }: {
    col: TableColumn;
    value: string;
...
export function TableFieldPreview({ field }: { field: FormField }) {
    ...
    const updateCell = (rowIdx: number, colId: string, value: string) => {
        setRows((prev) =>
            prev.map((r, i) => (i === rowIdx ? { ...r, [colId]: value } : r))
        );
    };
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            {/* <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed transition-all hover:bg-secondary disabled:opacity-40"
